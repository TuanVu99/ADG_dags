...
def update_chi_tieu(file, code, kenh, dong_sp):
    ...
    if check:
        ...
        fast_insert(data, table, schema, columns, sql_pre_insert=f"""DELETE FROM {schema}.{table} WHERE "Com.Code" = '{code}' {tmp_dong_sp if dong_sp else ''} {tmp_kenh if kenh else ''} and EXTRACT('Year' FROM "Kỳ") = '2023' """)
